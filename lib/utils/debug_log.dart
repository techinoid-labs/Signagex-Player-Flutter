...
// output goes nowhere visible. On Windows the exe is GUI-subsystem with no
// console attached; on Linux the player runs from a desktop launcher or as
// a service unit, so stdout is either discarded or buried in the journal.
...
//
// Rotate past a cap and keep one ".previous", the same scheme the Windows
// watchdog uses for its own log, with a large cap since this one is far
// more verbose. Worst case on disk is _kMaxLogBytes * 2. Keeping the previous file is
...
Future<void> debugLog(String tag, String message) {
  final next = _writeChain.then((_) async {
    try {
      ...
      // reinstall over a removed install (packaging deletes this exact
      // folder), starts with nothing here until something creates it.
      // File.writeAsString does NOT create
      // missing parent directories on either platform; it throws
      ...
      // Platform.pathSeparator rather than a hardcoded backslash: on Linux
